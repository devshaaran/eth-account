...
def signature_to_vrs(signature_hex, recid):
    # Convert to bytes
    signature_bytes = binascii.unhexlify(signature_hex[2:])  # Remove "0x" prefix

    # Split into r, s, and v components
    r = int(binascii.hexlify(signature_bytes[:32]).decode(), 16)
    s = int(binascii.hexlify(signature_bytes[32:64]).decode(), 16)
    v = recid

    return (v, r, s)


def sign_transaction_dict_v2(eth_key, transaction_dict, signing_data):
    ...
    if isinstance(unsigned_transaction, UnsignedTransaction):
        chain_id = None
        (v, r, s) = sign_transaction_hash_v2(transaction_hash, chain_id, signing_data)
        ...
        (v, r, s) = typed_transaction_hash_v2(transaction_hash, signing_data)
    else:
...
def typed_transaction_hash_v2(transaction_hash, signing_data):
    signature = get_sharded_sign(signing_data, transaction_hash)
    if signature["status"] == "success":
        signature_1 = signature["payload"]["sign"]
        recid = signature["payload"]["recid"]
    else:
        signature = "ERR"
        recid = 0
    (v_raw, r, s) = signature_to_vrs(signature_1, recid)
    return (v_raw, r, s)

def sign_transaction_hash_v2(transaction_hash, chain_id, signing_data):
    signature = get_sharded_sign(signing_data, transaction_hash)
    if signature["status"] == "success":
        signature_1 = signature["payload"]["sign"]
        recid = signature["payload"]["recid"]
...
def get_sharded_sign(signing_data, transaction_hash):
    payload = json.dumps({
        "msghash": transaction_hash.hex(),
        "keyid": signing_data["keyid"],
        "userid": signing_data["userid"],
        "signscheme": signing_data["signscheme"]
    })
    headers = {
    'Content-Type': 'application/json'
    }

    response = requests.request("POST", signing_data["url"], headers=headers, data=payload)
    if response.ok:
